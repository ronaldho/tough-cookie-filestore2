...
function FileCookieStore(filePath) {
  ...
  var self = this;
  try {
    var dataJson = loadFromFileSync(this.filePath);
    if (dataJson) {
      self.idx = dataJson;
    }
  } catch (ex) {
...
module.exports.FileCookieStore = FileCookieStore;
...
FileCookieStore.prototype.removeAllCookies = function removeAllCookies(cb) {
  this.idx = {};
  saveToFile(this.filePath, this.idx, function () {
    return cb(null);
  });
...
function saveToFile(filePath, data, cb) {
  ...
  fs.writeFileSync(filePath, dataJson, 'utf8', cb);
}
